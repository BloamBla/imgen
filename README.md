...
- **32 GB unified memory recommended** for full feature set; `imgen generate`'s default flipped to FLUX.2-klein-4b-edit at full bf16 in v0.11.2, so it now also wants **32 GB** (~22 GB peak) — 16 GB only with `--quantize 4` (degraded, as all klein is at low quant). **`imgen draw`'s default (FLUX.2-klein-4b at full bf16) needs 32 GB** (~22 GB peak) — on 16 GB use `imgen draw --model flux-dev --quantize 4`. **`imgen refine` requires 32 GB** (real measurement Q4/1536² peaks at ~23 GB resident + compression — 16 GB Macs would OOM mid-inference). **`imgen video` (LTX-Video) needs ~17 GB available** for the canonical 768×512 × 25-frame envelope — close Chrome/IDE before running on 32 GB Macs.
- **~50 GB free disk** for the default image stack (FLUX.2-klein-4B + klein-9B ≈ 47 GB cached). FLUX.1-Kontext / FLUX.1-dev re-download **~31 GB each only if** you pass `--model flux-kontext` / `flux-dev` (v0.11.2 dropped FLUX.1 from the default cache), and Qwen-Image-Edit downloads on first `--model qwen-image-edit-v1` use; **add ~26 GB more for video** (LTX-Video weights).
- **HuggingFace account** (the default image models — FLUX.2-klein-4B/9B — are gated: need a token + license acceptance; same for FLUX.1-Kontext/dev if you opt in). LTX-Video is **open** (no token, no license click-through).
...
imgen train --dataset <dir> --name alina --trigger "al1na woman" --base flux2-klein-9b  # v0.11.4: 9B base (48 GB+; thrashes on 32 GB)
...
When you pass `--lora <name>` on a model it isn't compatible with (e.g. a `flux-1` style LoRA on the default `flux2-klein-4b-edit` model, or a `flux2-klein-4b` LoRA on `--model flux-kontext`), `imgen` prints a warning naming the `--model` to re-run with — the LoRA is **not** silently applied. When a LoRA's trigger word isn't already in your prompt, `imgen` auto-prepends it and prints `Applied LoRA trigger '…'` so you can see it happen.
...
LoRAs are architecture-bound. A LoRA trained for FLUX.1 will NOT load on Qwen-Image-Edit, and vice versa. `imgen` declares a `lora_compat_group` on each model (`"flux-1"` for the `flux-kontext` model; `"flux2-klein-4b"` for the default `flux2-klein-4b-edit` i2i model and `flux2-klein-4b`; `"qwen"` for `qwen-image-edit-v1`); when a LoRA's `compatible_with` list doesn't include the active model's group, `imgen` warns once and skips the LoRA, then continues with the rest of the stack (or fully text-only if all LoRAs are incompatible). It doesn't crash and doesn't silently apply a mismatched LoRA.
...
- `flux-kontext` — **FLUX.1 Kontext Dev** — best quality for dedicated style transfer; **was the `imgen generate` / `batch` default through v0.11.1** (v0.11.2 flipped that to `flux2-klein-4b-edit`). Still selectable via `--model flux-kontext`, but its weights are **no longer cached by default**, so the first such run re-downloads (~31 GB). Gated, requires:
...
- `flux2-klein-4b` (**default for `imgen draw`** since v0.11.0) — **FLUX.2-klein-4B** distilled t2i — fast 4B base, also the `imgen train` LoRA base. **Runs at full bf16 by default** (its q8 t2i underwhelmed in our tests; full precision looked clearly better — ~22 GB peak, 32 GB Mac). **Distilled**: guidance pinned to 1.0, negative prompts dropped (explicit `--guidance != 1.0` rejected). Gated, accept license at https://huggingface.co/black-forest-labs/FLUX.2-klein-4B.
- `flux2-klein-4b-edit` (v0.11.1; **default for `imgen generate` / `batch` since v0.11.2**) — the same klein-4b in **edit / image-to-image** mode: `imgen generate <photo> [--lora <name>] --custom-prompt "..."` restyles/edits an input photo, optionally applying a klein-4b LoRA (e.g. a trained identity — it shares `flux2-klein-4b`'s LoRA compat group). Full bf16, same 32 GB envelope + gated repo as the base. **Note:** ships without Kontext's tuned identity-anchor enhance contract — for a dedicated restyle that must hard-preserve a face without a LoRA, `--model flux-kontext` may still do better.
...
- `flux2-klein-edit-9b` — **FLUX.2-klein-9B** distilled edit — default for `imgen refine` (v0.7.5+). Native ~4 MP support (up to 2048²) past FLUX.1's 1.5K clean ceiling. Gated, accept license at https://huggingface.co/black-forest-labs/FLUX.2-klein-9B. Q4 default needs **~24 GB peak RAM** (real measurement at 1.5K-2K²; 32 GB Mac required, 16 GB will OOM). Internal `--guidance` pinned to 1.0 by mflux — `imgen refine` handles this automatically (the `--guidance` flag still works for `--model flux-kontext` fallback). **Also selectable for `imgen generate` / `batch`** (`--model flux2-klein-edit-9b`) as a higher-quality i2i restyle than the 4B default — smarter on detail, but heavier (q8, ~slower) and a **different LoRA compat group** (`flux2-klein-9b`), so a klein-4b identity LoRA like `stas` will NOT load on it; use the 4B `flux2-klein-4b-edit` default for LoRA-driven edits.
- `flux2-klein-9b` (v0.11.4) — **FLUX.2-klein-9B** distilled t2i, the 9B sibling of `flux2-klein-4b` (same weights as the edit-9b refine row — no extra download). Heavier / higher-detail t2i than the 4B default: `imgen draw --model flux2-klein-9b`. **`default_quantize=8`** (NOT 16 — 9B@q16 won't fit 32 GB, and unlike 4B the 9B holds quality at q8: a real smoke produced a strong photoreal coastal scene at ~20 GB resident; 48 GB+ can pass `-q 16`). Also a **2nd LoRA-training base** — `imgen train --base flux2-klein-9b`. **RAM reality:** the real training step peaks **~37.6 GB resident + ~18.6 GB swap**, so on a 32 GB Mac it thrashes (~8× slower, ~50 s/it) — *works but not productive*; **48 GB+ for real training** (the preflight gates 32 GB at ~45 GB needed; `--force` to try anyway). Distilled: guidance pinned 1.0, no negatives. Gated (same FLUX.2-klein-9B repo as `imgen refine`).
...
**FLUX.2-klein-4b-edit is the i2i default (since v0.11.2); FLUX.1-Kontext stays selectable.** FLUX.2 (released Nov 2025; klein distilled variants Jan 2026) splits into families: `klein-base` is text-to-image (no input photo), and `klein-edit` is *instruction-based editing* ("make the sky blue") plus i2i. v0.11.2 made `flux2-klein-4b-edit` the `imgen generate` / `imgen batch` default as part of the FLUX.1→FLUX.2 migration — a real M2 Pro smoke confirmed it preserves identity well on a no-LoRA restyle, and it shares the klein-4b LoRA compat group so a trained identity LoRA can edit a photo. FLUX.1-Kontext was purpose-built for "rewrite this image while preserving identity / pose / composition" and may still edge out klein-4b-edit on a dedicated hard-preserve restyle, so it stays selectable via `--model flux-kontext` (weights re-download on first use since v0.11.2 dropped FLUX.1 from the default cache). The six built-in `--style` presets ride flux-1 (Kontext) LoRAs + Kontext's verb conventions ([BFL Kontext prompting guide](https://docs.bfl.ai/guides/prompting_guide_kontext_i2i)); on the default klein-4b-edit they warn-and-skip to text-only, so pass `--model flux-kontext` for the curated style look. v0.7.5 added `imgen refine` on **FLUX.2-klein-edit-9b** (via `mflux-generate-flux2-edit`) because the Hires-Fix workload — preserve composition, push detail, upsample past 1.5K — wants exactly the low-strength i2i FLUX.2-klein-edit is good at, AND its native 4 MP support clears FLUX.1's clean 1.5K ceiling without tiling artifacts. You can still swap refine onto `--model flux-kontext` if you want to stay under 1.5K with Kontext.
